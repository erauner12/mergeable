...
export function formatPromptBlock(block: PromptBlock): string {
  if (block.kind === "diff") {
    ...
    const parts: string[] = [];
    if (block.diffHunk) { // Only threads from review comments will have diffHunk
      parts.push("```diff");
    ...
    if (block.threadId) { // This indicates it came from makeThreadBlock or represents a comment thread
      parts.push(block.commentBody); // This body is already fully formatted with individual comments
    } else { // Simple comment (e.g. issue comment, review summary, or PR description)
